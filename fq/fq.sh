...
main() {
    ...
    if [[ "$is_first" =~ ^[Yy]$ || -z "$is_first" ]]; then
        ...
        if [[ "$has_domain" =~ ^[Yy]$ || -z "$has_domain" ]]; then
            echo "👉 将通过 nginx(443) -> xray(6443) 的方式安装"
            ...
            if [[ "$is_cf" =~ ^[Yy]$ || -z "$is_cf" ]]; then
                ...
                echo "---"
                echo "将使用 certbot 帮您申请 *.$domain 的证书"
                sudo certbot certonly --manual --preferred-challenges dns --agree-tos --no-eff-email -d "*.$domain" -d "$domain" || return 1
                echo "✅ 证书申请完成！"
            fi

            update_nginx_config "$domain" "$subdomain" || return 1
            install_xray 6443 || return 1
        else
            echo "👉 将通过 xray(443) 的方式安装"
